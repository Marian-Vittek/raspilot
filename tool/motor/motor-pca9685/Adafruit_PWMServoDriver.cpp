 ...
 */
/*
  Deliberately modified for raspbery Pi by Marian Vittek
 */
...
#define delay(n) usleep(n*1000LL);
...
Adafruit_PWMServoDriver::Adafruit_PWMServoDriver()
    : _i2caddr(PCA9685_I2C_ADDRESS), _i2c(-1) {}
...
void Adafruit_PWMServoDriver::begin(uint8_t prescale) {
  if (_i2c < 0) _i2c = pi2cOpen((char*)"/dev/i2c-1", _i2caddr);
  reset();
  if (prescale) {
    setExtClk(prescale);
  } else {
 ...
 *          Sets External Clock (Optional)
 */
void Adafruit_PWMServoDriver::end() {
  if (_i2c >= 0) pi2cClose(_i2c);
  _i2c = -1;
}
...
uint8_t Adafruit_PWMServoDriver::setPWM(uint8_t num, uint16_t on,
  ...
  i = 0;
  b[i++] = PCA9685_LED0_ON_L + 4 * num;
  b[i++] = on;
  b[i++] = on >> 8;
  b[i++] = off;
  b[i++] = off >> 8;
  r = pi2cWrite(_i2c, b, i);
  return (r>0 ? 0 : 4);
}
...
#ifdef ENABLE_DEBUG_OUTPUT
  printf("Setting PWM Via Microseconds on output");
  printf("%d", num);
  printf(": ");
