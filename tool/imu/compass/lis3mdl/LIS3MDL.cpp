...
bool LIS3MDL::init(char *devicePath, deviceType device, sa1State sa1)
{
  ...
  fd_i2c = pi2cOpen(devicePath, address);
...
{
  pi2cWriteBytesToReg(fd_i2c, reg, 1, &value);
...
{
  uint8_t value;
  pi2cReadBytes(fd_i2c, reg, 1, &value);
  return value;
}
...
void LIS3MDL::read()
{
  ...
  pi2cReadBytes(fd_i2c, OUT_X_L, 6, x);
...
{
  uint8_t val;
  pi2cReadBytes(fd_i2c, reg, 1, &val);
  return(val);
}
