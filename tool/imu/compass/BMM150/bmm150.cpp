...
int8_t BMM150::initialize(char *devicePath) {
  fd_i2c = pi2cOpen(devicePath, BMM150_I2C_Address);
    ...
    usleep(1000*BMM150_START_UP_TIME);
...
void BMM150::soft_reset() {
    ...
    usleep(1000*BMM150_SOFT_RESET_DELAY);
}
...
void BMM150::i2c_write(short address, short data) {
  uint8_t b;
  b = data;
  pi2cWriteBytesToReg(fd_i2c, address, 1, &b);
}

void BMM150::i2c_read(short address, uint8_t* buffer, short length) {
  // pi2cReadBytes(int fd, uint8_t regAddr, uint8_t length, uint8_t *data);
  pi2cReadBytes(fd_i2c, address, length, buffer);
}


void BMM150::i2c_read(short address, int8_t* buffer, short length) {
  // pi2cReadBytes(int fd, uint8_t regAddr, uint8_t length, uint8_t *data);
  pi2cReadBytes(fd_i2c, address, length, (uint8_t *)buffer);
}

uint8_t BMM150::i2c_read(short address) {
  uint8_t byte;
  pi2cReadBytes(fd_i2c, address, 1, &byte);
  return(byte);
}
