...
int mspScanMessage(FILE *ff) {
    ...
    int 	quality;
    int32_t 	distanceMm, motionX, motionY;
    int 	ck;
    uint8_t 	checksum;
	...
	distanceMm = (int32_t)(getccrc(ff, &checksum) + (getccrc(ff, &checksum) << 8)  + (getccrc(ff, &checksum) << 16) + (getccrc(ff, &checksum) << 24));
	ck = getc(ff);
	if (ck != checksum) {
	    printf("%s:%d: Error: wrong checksum.\n", __FILE__, __LINE__);
	    return(-1);
	}
	printf("range %5.3f %4.2f\n", distanceMm/1000.0, quality/255.0);
	...
	printf("motion %5.3f %5.3f %4.2f\n", motionX/1000.0, motionY/1000.0, quality/255.0);
